...
ALTER TABLE "ai_model_configs" ADD CONSTRAINT "ai_model_configs_provider_protocol_check" CHECK (provider_protocol = ANY (ARRAY['creator_dev'::text, 'openai_images'::text, 'openai_compatible_chat'::text, 'cumob_chat'::text, 'modelflare_responses'::text, 'volcengine_ark_image'::text, 'volcengine_ark_video'::text, 'aliyun_bailian_video'::text, 'aliyun_bailian_audio'::text, 'apimart_audio'::text, 'globalaiopc_video'::text, 'globalaiopc_sound_clone'::text, 'lingdong_api'::text, 'cumob_image'::text, 'global_ai_opc_image'::text, 'extra_token_video'::text, 'saier_video'::text, 'banana_router'::text, 'san_bao'::text, 'custom_http'::text]));
