...
> 适用对象：第一次使用灵曦AI完成短剧、漫剧或分镜视频创作的用户。
> 更新时间：2026-07-09
...
打开灵曦AI后，先在登录页完成账号登录。登录成功后会进入创作工作台。
...
需要下载分镜视频时，先勾选要导出的分镜，再点击右上角“导出”。导出弹窗提供 MP4 和剪映工程文件入口，两种方式都只处理当前已勾选的分镜。

怎么做：

1. 在分镜卡片左上角勾选要导出的分镜，也可以使用“全选”。
2. 点击“导出”。未勾选时系统会提示“请先选择分镜”。
3. 选择 `MP4` 或“剪映工程文件”。
4. 等待系统检查已勾选分镜的视频完整性。

选择 `MP4` 后会下载一个 ZIP，里面是每条已勾选分镜的当前视频；选择“剪映工程文件”后会下载一个 ZIP 草稿包。两个包内的视频均按 `序号-剧集名-分镜名` 命名。剪映 ZIP 下载完成后，请按照[剪映草稿包导入指南](how-to-import-jianying-draft.md)解压并放入剪映草稿位置。

完成标志：

- 如果所有已勾选分镜都有当前视频，系统会生成可下载内容。
- 如果已勾选分镜缺少当前视频，系统会提示不能导出；未勾选分镜不影响本次导出。

## 15. 下载视频或处理缺失项

![导出检查](assets/creator-guide/15-export-complete.png)

导出前系统会检查已勾选分镜是否满足导出条件。截图里的状态表示所选素材还没有满足导出条件，这是正常的保护机制。

怎么做：

1. 如果出现下载入口，点击下载。MP4 和剪映工程的下载结果均为 `.zip` 包。
2. 如果出现“缺少视频”“请稍后重试”等提示，先回到分镜列表检查已勾选分镜。
3. 对缺少视频的已勾选分镜，重新生成并设为分镜视频，或取消勾选后导出其他分镜。
4. 所选分镜完成后，再次导出。

完成标志：

- MP4 视频 ZIP 或剪映草稿 ZIP 可以下载。
- ZIP 中只包含已勾选分镜，视频名称能看出所属剧集和分镜。
...
8. 先生成分镜图，并设为分镜图。
9. 再用首帧图生成视频，并设为分镜视频。
10. 勾选需要的分镜后，导出 MP4 视频 ZIP 或剪映工程 ZIP。
