...
| --- | --- | --- |
| **Identity/Auth** | 用户、登录、会话、认证适配器 | 组织额度、项目权限 | 后续迁移到独立服务 |
...
| **Model-Gateway** | 供应商适配器、供应商能力矩阵、供应商请求记录、路由/回退、错误归一化 | 创作者工作流状态、额度策略 | 后续迁移到独立服务 |
| **Commerce-Payment** | 积分包、订单、支付单、支付渠道事件、退款、开票元数据 | 积分余额、生成消耗、模型供应商调用 | 后续迁移到独立服务 |
| **Credit-Billing** | 额度账本、额度预留、供应商成本条目、余额读模型 | 供应商 HTTP 调用 | 后续迁移到独立服务 |
...
GET    /admin/tasks                             # 任务查询
POST   /admin/tasks/:taskId/retry               # 手动重试
POST   /admin/providers/:providerId/disable     # 禁用供应商
GET    /admin/costs                             # 成本/利润视图
GET    /admin/audit                             # 异常事件审计 补充异常事件的审核规则
暴露补偿积分的接口（如 `/admin/organizations/:orgId/credits/adjust`），但不暴露修改余额的接口
```
...
// 人工客服退款
// 先不做 payment.refund.succeeded      Commerce-Payment      Credit-Billing / Admin-Ops（退款后额度调整/审计）
// 支付超时延迟对列，直接关单
...
| P0-A | 对象存储不可用 | 新上传/生成失败，旧资产如已有 CDN 缓存可访问 | 任务失败且保留错误原因，不静默成功 | 打电话到负责人
| P0-B | 消息队列积压 | Worker 持续消费，新的 Outbox 事件持续入队 | 不影响 API 创建事实记录，通过 Outbox 解耦 | 发短信/发消息到监控群
| P0-B | Redis 完全丢失 | BullMQ job 丢失，但任务记录在 PostgreSQL 中安全 | 修复分发器扫描 `queued` 任务重建 BullMQ job | 后续升级到kafka
| P0-B | 缓存雪崩 | 本项目不使用 Redis 缓存业务数据 | PostgreSQL 通过连接池和索引抗读取压力 |
| P0-B | 第三方（供应商）超时 | accept 前按安全策略重试；accept 后进入 `result_unknown` | Worker 超时配置 + 供应商适配器声明超时行为 + 对账 | 备用供应商(官方)+退积分
| P0-B | Worker 崩溃 | 租约过期后修复作业接管 | 租约（`locked_until` + `heartbeat_at`）机制 |
| P0-B | 数据库主库故障 | 应用不可用（单区域限制） | 依赖托管数据库的高可用能力 | 数据库自动故障转移 |
...
- P0 使用自建身份系统，P0 主路径采用手机号验证码登录，不依赖第三方认证供应商的数据模型
- 用户、组织、成员资格、角色和项目权限存储在平台数据库中
- 密码、OAuth 微信 和 SSO 通过适配器接口预留，未来可插拔集成
...
| **Redis/PostgreSQL 裂脑** | 任务丢失或重复 | 中 | 事务性 Outbox + 修复分发器 + Worker 幂等 | PostgreSQL 是单一数据源
