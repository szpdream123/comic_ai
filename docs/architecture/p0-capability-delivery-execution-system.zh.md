...
- 使用真实的结构化日志，至少携带 `traceId`、`userId`、`projectId` 以及适用的任务/工作流 ID。
...
| 可观测性 | `traceId`、`userId`、拒绝原因 |
| 测试 | 401、403、禁用用户/成员、跨用户访问负面测试 |
...
| 异常场景 | 无效输入、缺少项目、禁止、重复键冲突 |
| 幂等性 | `(user_id, project.create, idempotency_key)` 带请求哈希 |
| 安全性 | `project:create`；项目所有权或明确的团队成员分配 |
| 可观测性 | `traceId`、`userId`、`projectId`、参与者 |
| 测试 | 成功、无效输入、重放、冲突、禁止、跨用户访问 |
...
| --- | --- | --- | --- |
| 跨用户访问 | 任何没有用户或项目授权的查询 | 用户/项目授权辅助 | 阻止项目/资产 PR |
