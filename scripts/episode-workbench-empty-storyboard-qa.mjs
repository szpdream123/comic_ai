...
const managedServer =
  process.env.QA_ORIGIN ? null : await startManagedBackendServer(Number(process.env.QA_PORT ?? 4314));
...
const artifactDir = resolve(process.cwd(), "artifacts", "episode-workbench-empty-qa");
const userDataDir = resolve(
  process.cwd(),
  ".local",
  `episode-workbench-empty-qa-chrome-${Date.now()}-${randomUUID()}`,
);
...
try {
  ...
  const workbenchUrl = `${origin}/app.html#/projects/${encodeURIComponent(setup.projectId)}/episodes/${encodeURIComponent(setup.emptyEpisodeId)}`;
  await navigate(page, workbenchUrl);
  await waitForSelector(page, ".episode-replica-layout.storyboard-mode", 15_000);

  await screenshot(page, join(artifactDir, "step-32a-empty-storyboard.png"));
  const emptyMetrics = await evaluate(page, emptyStoryboardMetricsScript());

  await click(page, '[data-action="add-storyboard"]');
  await waitForCondition(
    page,
    "document.body.innerText.includes('共 1 条') && document.body.innerText.includes('0 / 5000')",
    10_000,
  );
  await screenshot(page, join(artifactDir, "step-32b-first-storyboard.png"));
  const firstStoryboardMetrics = await evaluate(page, emptyStoryboardMetricsScript());
  ...
  await screenshot(page, join(artifactDir, "step-32c-empty-prompt-toast.png"));
  const emptyPromptMetrics = await evaluate(page, emptyStoryboardMetricsScript());

  const report = {
    origin,
    projectId: setup.projectId,
    episodeId: setup.emptyEpisodeId,
    workbenchUrl,
    screenshots: {
      emptyStoryboard: "artifacts/episode-workbench-empty-qa/step-32a-empty-storyboard.png",
      firstStoryboard: "artifacts/episode-workbench-empty-qa/step-32b-first-storyboard.png",
      emptyPromptToast: "artifacts/episode-workbench-empty-qa/step-32c-empty-prompt-toast.png",
    },
    metrics: {
      emptyStoryboard: emptyMetrics,
      firstStoryboard: firstStoryboardMetrics,
...
async function setupScenario() {
  ...
  const project = await jsonFetch("/api/creator/project/create", {
    method: "POST",
    cookie,
    idempotencyKey: `qa-empty-project-${Date.now()}`,
    body: {
      name: "空分镜工台联调",
      scriptInput: "第 1 集：验证分镜空态与首条创建。",
      aspectRatio: "9:16",
      resolution: "1080p",
    },
  });
  ...
  const episodeEnvelope = await jsonFetch(`/api/projects/${project.project.id}/episodes`, {
    method: "POST",
    cookie,
    idempotencyKey: `qa-empty-episode-${Date.now()}`,
    body: { title: "空白分镜集" },
  });
  return {
    cookie,
    projectId: project.project.id,
    emptyEpisodeId: episodeEnvelope.data.episode.id,
  };
}
...
function emptyStoryboardMetricsScript() {
  return `(() => ({
    bodyText: document.body.innerText,
    promptLength: document.querySelector('#video-prompt-input')?.value?.length ?? 0,
    storyboardCountText: document.body.innerText.includes('共 0 条') ? '0' : (document.body.innerText.includes('共 1 条') ? '1' : 'unknown'),
    hasEmptyState: document.body.innerText.includes('当前剧集还没有分镜'),
    ...
    hasResultAction: Boolean(document.querySelector('[data-result-action="set-storyboard-image"]')),
  }))()`;
}
...
function validateReport(report) {
  ...
  if (!report.metrics?.emptyStoryboard?.hasEmptyState) {
    failures.push("empty storyboard state did not render");
  }
  if (report.metrics?.emptyStoryboard?.storyboardCountText !== "0") {
    failures.push(`expected empty storyboard count 0, got ${report.metrics?.emptyStoryboard?.storyboardCountText}`);
  }
  if (!String(report.metrics?.emptyStoryboard?.stageTitle ?? "").startsWith("分镜：")) {
    failures.push(`empty storyboard stage title mismatch: ${report.metrics?.emptyStoryboard?.stageTitle}`);
  }
  if (!report.metrics?.emptyStoryboard?.hasPromptPlaceholder) {
    failures.push("empty storyboard prompt placeholder missing");
  }
  if (report.metrics?.firstStoryboard?.storyboardCountText !== "1") {
    failures.push(`expected first storyboard count 1, got ${report.metrics?.firstStoryboard?.storyboardCountText}`);
  }
  if (report.metrics?.firstStoryboard?.promptLength !== 0) {
    failures.push(`first storyboard prompt should stay empty, got length ${report.metrics?.firstStoryboard?.promptLength}`);
  }
  if (report.metrics?.firstStoryboard?.hasResultAction) {
    failures.push("first storyboard should not pre-render generation results");
  }
  if (!report.metrics?.emptyPromptToast?.hasValidationToast) {
    failures.push("empty prompt toast did not render");
  }
  if (report.metrics?.emptyPromptToast?.hasResultAction) {
    failures.push("empty prompt generate should not create result actions");
  }
  if (failures.length) {
    throw new Error(`episode_workbench_empty_storyboard_qa_failed\n- ${failures.join("\n- ")}`);
  }
}
