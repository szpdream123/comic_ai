...
const userId = "10000000-0000-4000-8000-000000050001";
...
describe("credit lots", { concurrency: false }, () => {
  ...
  it("expires available credit lots idempotently", async () => {
    ...
    try {
      ...
      const ledger = await db.query<{ entry_type: string; amount: number; available_delta: number; balance_after: number }>(
        "SELECT entry_type, amount, available_delta, balance_after FROM credit_ledger_entries WHERE entry_type = 'expire'",
      );
      ...
      assert.deepEqual(ledger.rows, [{ entry_type: "expire", amount: 100, available_delta: 0, balance_after: 100 }]);
...
async function seedUser(
  ...
  await db.query(
    `
      ...
      VALUES ($1, '13800138000', 'active', $2, 0, 0)
    `,
    [userId, balance],
  );
}
