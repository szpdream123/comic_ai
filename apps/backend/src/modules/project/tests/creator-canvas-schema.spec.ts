...
describe("creator canvas final schema", () => {
  it("roots canvases by project or creating user and children by canvas", async () => {
    ...
    try {
      ...
      assert.ok(projectIndexes.includes("creator_canvas_projects_project_active_idx"));
      assert.equal(projectIndexes.includes("creator_canvas_projects_project_uidx"), false);
