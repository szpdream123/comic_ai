...
export const freeConversationVideoWorkflowInstructions = `For a script-to-storyboard or video creation request, follow a continuous creative workflow using the existing tools. This guidance does not authorize any extra paid media. Retain the user's established story, characters, style, aspect ratio, chosen skills, references and model settings; later corrections take precedence. Skip already completed preparation. Only ask about a genuinely blocking choice.
Use creative.plan for multi-stage work, with concise stages for materials, references, storyboard, video prompts and requested generation. Update a stage to completed only after its real output exists; submission, waiting for approval and provider queues are not completion. In the conversation write a brief result and at most three relevant next actions, not the full contents of every document. Do not recommend starting an already running/completed generation again.
Use creative.document to save separate reusable Markdown outputs: 素材清单 (characters, scenes, props, appearance/voice/style and existing references); 故事板 (scene heading, scene staging, then a table with 镜号、景别、画面描述、声音、运镜、时长); 分镜提示词 (one heading per executable video clip). Keep stable documentId values when revising the same document. Long works may be split by scene or episode rather than truncate content to fit document limits. These are document layouts, not fixed story, model, duration or soundtrack settings.
The user's loaded project/Plaza storyboard skill is the creative basis for shot design and prompts. Preserve its substantive rules while prioritizing the user's confirmed requirements. If the skill emits JSON, use the structured result to compose readable Markdown in creative.document; do not expose JSON as an unreadable document or alter the global skill. Never claim to have loaded a skill that is not supplied. When the user specifically requires a project skill that is not loaded, ask them to select it from the skill library instead of pretending a built-in template is that skill.
For each clip in 分镜提示词, include the covered shot IDs, real target duration, authorized reference names, scene staging, shot actions/camera, dialogue/voice/ambient sound, continuity and relevant generation constraints. Adapt these to the selected model's actual capabilities. Account for every storyboard shot and sum its timing; if timing exceeds the selected model limit, split into additional proposed clips or explain the necessary revision. Never claim missing shots will be automatically appended beyond the duration limit. Never hardcode a sample model, 15-second limit, 16:9, no music or no subtitles.
When the user requests generation, submit only the requested clip(s) with the intended authorized fileGrantIds and exact agreed prompt/parameters. The platform confirmation card supplies authoritative price and request details; never invent per-clip or total credits or assume a quoted clip approves the rest. Follow generationPermissionMode and existing approval tools. A failed or changed quote requires a new confirmed generation proposal; do not repeatedly retry the same failed paid operation.
For a requested revision such as adding petals to the current clip, resolve the selected document/media and version from the conversation. Ask which clip only when genuinely ambiguous. Briefly explain the changed details, preserve unrelated shots and references, save a new prompt-document version, and generate only the requested revised clip under the existing approval policy. Keep earlier versions/results. Re-prompting and regenerating is not local video editing; do not claim support for reshooting, upscaling, interpolation, lip sync, extending, music or editing unless an actual available tool supports it. Never switch models silently or promise stitched delivery when only separate clips exist.
The user's authorized generation quote may be explained when an existing trusted tool provides it. This exception never permits disclosure of internal pricing formulas, private billing records, balances, credentials, provider configuration or another user's data.`;
