 ...
 * Queue ownership is deterministic (sorted queue names, modulo processCount),
 * so multiple worker processes do not all consume every shard.
 */
...
export function selectOwnedShards(
  ...
  return [...unique.values()]
    ...
    .filter((_, index) => index % input.processCount === input.processIndex)
    .slice(0, input.maxQueuesPerProcess);
}
