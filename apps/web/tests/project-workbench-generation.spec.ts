...
describe("episode workbench asset list layout", () => {
  ...
  it("renders quick-lane preview images as full-card backgrounds", () => {
    const css = readFileSync(
      new URL("../src/features/production-workbench/production-workbench.css", import.meta.url),
      "utf8",
    );
    const finalQuickLaneRules = css.match(
      /\/\* Final right quick-lane thumbnail rules \*\/(?<body>[\s\S]*?)\/\* Final override for left-side asset tabs/,
    )?.groups?.body ?? "";

    assert.match(finalQuickLaneRules, /\.episode-replica-quick-asset \.thumb\s*\{[\s\S]*?position:\s*absolute/);
    assert.match(finalQuickLaneRules, /\.episode-replica-quick-asset \.thumb\s*\{[\s\S]*?inset:\s*0/);
    assert.match(finalQuickLaneRules, /\.episode-replica-quick-thumb-image\s*\{[\s\S]*?width:\s*100%/);
    assert.match(finalQuickLaneRules, /\.episode-replica-quick-thumb-image\s*\{[\s\S]*?height:\s*100%/);
    assert.match(finalQuickLaneRules, /\.episode-replica-quick-thumb-image\s*\{[\s\S]*?object-fit:\s*cover/);
  });
  ...
  it("clears stale local storyboards when a fresh workbench storyboard reload comes back empty", () => {
    const source = readFileSync(
      new URL("../src/features/production-workbench/index.js", import.meta.url),
      "utf8",
    );
    ...
    assert.doesNotMatch(loadStoryboardsBlock, /return existingStoryboards;/);
    assert.match(loadStoryboardsBlock, /if \(!mappedStoryboards\.length\) \{/);
    assert.match(loadStoryboardsBlock, /\[episodeId\]: \[\]/);
    assert.match(loadStoryboardsBlock, /return \[\];/);
  });

  it("does not auto-create a timestamp draft storyboard for empty persisted episodes", () => {
    const result = syncEpisodeStoryboardMapForTest(
      {
        "episode-empty": [],
      },
      [],
    ...
    assert.deepEqual(result["episode-empty"], []);
  });
  ...
  it("hydrates episode assets from the workbench api when entering asset scope or switching asset tabs", () => {
    ...
    assert.match(setScopeBlock, /void hydrateEpisodeWorkbenchScopeAfterSwitch\(workbench,\s*nextScopeMode\)/);
...
describe("production workbench project tab", () => {
  ...
  it("updates the prompt dock when selecting image reference modes from the mini menu", async () => {
    ...
    const workbench = {
      ...
      ui: buildProjectUi({
        ...
        importedAssets: {
          character: [
            {
              id: "character-reference-mode",
              name: "测试角色",
              description: "测试角色描述",
              previewUrl: "/uploads/character-reference-mode.png",
            },
          ],
          scene: [],
          prop: [],
        ...
        assetPromptDraft: {
          quickReferenceItems: [
            {
              id: "asset-quick-reference-1",
              name: "资产快捷引用图",
              kind: "image",
              url: "/uploads/asset-quick-reference.png",
            },
          ],
          referenceUploads: [
            {
    ...
    assert.match(singleHtml, /文生图/);
    assert.match(singleHtml, /data-action="quick-append-selected-asset"/);
    assert.doesNotMatch(singleHtml, /asset-quick-reference\.png/);
    assert.doesNotMatch(singleHtml, /asset-reference-upload\.png/);
  ...
  it("defers supplemental episode workbench requests until after required context hydration", async () => {
    let resolveWorkbenchContext;
    const pendingWorkbenchContext = new Promise((resolve) => {
      resolveWorkbenchContext = resolve;
    });
  ...
  it("skips generation task restoration when opening the asset workbench scope", async () => {
    let resolveWorkbenchContext;
    const pendingWorkbenchContext = new Promise((resolve) => {
      resolveWorkbenchContext = resolve;
    });
  ...
  it("renders the batch image modal with Muse-like grouped controls instead of a placeholder shell", () => {
    ...
    assert.match(html, /\[系统\]场景-广角图/);
    assert.match(html, /\[系统\]角色-三视图/);
    assert.match(html, /\[系统\]道具-三视图/);
    assert.match(html, /nano banana 2（链路G）/);
  ...
  it("renders backend batch styles inside the public batch style tab", () => {
    ...
    assert.match(html, /生成1张图 \| 90 积分/);
    assert.match(html, /episode-batch-style-card selected/);
  });
  ...
  it("shows image-only batch entry label in assets scope", () => {
    const html = renderProductionWorkbench({
      state: buildProjectState(),
      session: { user: { phone: "+86 13800138000" } },
      ui: {
  ...
  it("fails asset batch image submission when the model task api is unavailable", async () => {
    const previousDocument = globalThis.document;
    globalThis.document = {
      body: {
        classList: {
  ...
  it("prepends the selected public style prompt only to the submitted batch conversation", async () => {
    const previousDocument = globalThis.document;
    globalThis.document = {
      body: {
        classList: {
          ...
          items: [
            {
              ...
              name: "任小草",
              kind: "character",
              description: "角色设计展示图，纯米白色背景，左侧为角色上半身特写肖像。",
            },
          ],
        },
      }),
    ...
    try {
      ...
      assert.equal(workbench.ui.batchImageStyles?.[0]?.prompt_content, "国风仙侠批量生图风格，保留东方服饰细节、仙侠氛围、清晰主体和统一光影质感。");
      assert.equal(workbench.ui.episodeBatchResults?.["character-1"]?.promptPreview, [
        "国风仙侠批量生图风格，保留东方服饰细节、仙侠氛围、清晰主体和统一光影质感。",
  ...
  it("submits asset batch image generation through the model task api instead of local placeholder images", async () => {
    ...
    try {
      ...
      assert.equal(createImageTaskCalls.length, 1);
      assert.equal(createImageTaskCalls[0].episodeId, "episode-new");
      assert.equal(createImageTaskCalls[0].payload.model, "gpt-image-2-cn");
      assert.equal(createImageTaskCalls[0].payload.targetType, "asset");
      assert.equal(createImageTaskCalls[0].payload.targetId, "character-1");
      assert.equal(createImageTaskCalls[0].payload.prompt, "国风仙侠批量生图风格，保留东方服饰细节。\n角色设计展示图，纯米白色背景。");
      assert.equal(createImageTaskCalls[0].payload.promptOverride, createImageTaskCalls[0].payload.prompt);
      assert.equal(createImageTaskCalls[0].payload.parameters.aspectRatio, "9:16");
      assert.equal(createImageTaskCalls[0].payload.parameters.quality, "2K");
  ...
  it("starts all asset batch image task submissions before waiting for previous responses", async () => {
    ...
    const workbench = {
      ...
      session: { user: { phone: "+86 13800138000" } },
      api: {
        createImageTask(_episodeId, payload) {
          startedTargetIds.push(payload.targetId);
  ...
  it("reconciles a cached pending asset generation result when selecting the asset again", async () => {
    ...
    assert.deepEqual(taskRequests, ["task-prop-queued"]);
    assert.equal(workbench.ui.imageGenerationResult?.status, "completed");
    assert.equal(workbench.ui.imageGenerationResult?.taskId, "task-prop-queued");
    assert.equal(workbench.ui.imageGenerationResult?.fixedImages?.[0]?.url, "https://example.com/generated-prop.png");
    assert.equal(workbench.ui.episodeBatchResults?.["prop-1"]?.status, "completed");
  });
  ...
  it("loads backend batch image styles when opening the asset batch image modal", async () => {
    const previousDocument = globalThis.document;
    globalThis.document = {
      body: {
        classList: {
    ...
    try {
      ...
      assert.equal(workbench.ui.batchImageStyles?.length, 2);
      assert.equal(
        workbench.ui.batchImageStyles?.[0]?.prompt_content,
  ...
  it("loads asset batch image model options and styles in parallel when opening the modal", async () => {
    const previousDocument = globalThis.document;
    globalThis.document = {
      body: {
        classList: {
    ...
    let resolveBatchStyles;
    const pendingModelOptions = new Promise((resolve) => {
      resolveModelOptions = resolve;
    });
    const pendingBatchStyles = new Promise((resolve) => {
      resolveBatchStyles = resolve;
    });
    const calls = [];
    const workbench = {
      state: {
      ...
      api: {
        async listBatchImageModelOptions() {
          calls.push("batch-model-options:start");
          const payload = await pendingModelOptions;
          calls.push("batch-model-options:done");
          return payload;
        },
        async getBatchImageStyles() {
          calls.push("batch-styles:start");
          const payload = await pendingBatchStyles;
          calls.push("batch-styles:done");
          return payload;
        },
      },
      ui: buildProjectUi({
    ...
    try {
      ...
      assert.deepEqual(calls, ["batch-model-options:start", "batch-styles:start"]);
      assert.equal(workbench.ui.episodeBatchModal ?? null, null);
      ...
      assert.deepEqual(calls, [
        "batch-model-options:start",
        "batch-styles:start",
        "batch-model-options:done",
        "batch-styles:done",
      ]);
      assert.equal(workbench.ui.episodeBatchModal?.show, true);
      ...
      resolveBatchStyles?.({ data: [] });
      globalThis.document = previousDocument;
    }
  });

  it("keeps the full generation config when loading narrow batch image model options", async () => {
    const previousDocument = globalThis.document;
    globalThis.document = {
      body: {
        classList: {
    ...
    try {
      ...
      assert.equal(workbench.ui.episodeBatchModal?.imageModelId, "batch-image-model");
    } finally {
      globalThis.document = previousDocument;
    }
  });
  ...
  it("calculates asset batch image credits from the selected backend model", async () => {
    ...
    const workbench = {
      ...
      session: { user: { phone: "+86 13800138000" } },
      api: {
        async listGlobalBatchImageModelOptions(options) {
          freshCalls.push(options);
          return {
            models: [
              {
                modelId: "priced-image-a",
                modelName: "定价图片A",
                ratios: ["16:9"],
                qualities: ["2K"],
              },
              {
                modelId: "priced-image-b",
                modelName: "定价图片B",
                ratios: ["16:9"],
                qualities: ["2K"],
              },
    ...
    try {
      ...
      assert.deepEqual(freshCalls, [{ fresh: true }]);
  ...
  it("renders selectable aspect ratio and clarity controls in the image batch modal", async () => {
    const previousDocument = globalThis.document;
    globalThis.document = {
      body: {
        classList: {
    ...
    const workbench = {
      ...
      session: { user: { phone: "+86 13800138000" } },
      api: {
        async listGlobalBatchImageModelOptions() {
          return {
            models: [
              {
                modelId: "priced-image-a",
    ...
    try {
      ...
      assert.equal(workbench.ui.episodeBatchModal?.imageClarity, "medium");
      assert.match(renderProductionWorkbench(workbench), /9:16/);
      assert.match(renderProductionWorkbench(workbench), /medium/);
    } finally {
      globalThis.document = previousDocument;
    }
  });
  ...
  it("does not show a success toast when clicking canvas operation nodes", async () => {
    const workbench = {
      state: buildProjectState(),
      api: {},
      ui: buildProjectUi({
    ...
    assert.equal(workbench.ui.canvasDocument.nodes.at(-1).type, "video");
    assert.equal(workbench.ui.toast, "");

    workbench.ui.toast = "之前的提示";

    await handleWorkbenchActionForTest(workbench, {
      dataset: {
        action: "add-canvas-template",
        nodeKind: "send",
        templateId: "template-send-image",
        assetId: "asset-1",
      },
      textContent: "角色参考图",
    });

    const addedTemplateNode = workbench.ui.canvasDocument.nodes.at(-1);

    assert.equal(addedTemplateNode.type, "send");
    assert.equal(workbench.ui.selectedCanvasNodeId, addedTemplateNode.id);
    assert.equal(workbench.ui.toast, "");
  });
  ...
  it("writes completed canvas image task results back into the generation preview", async () => {
            ...
            generatedOutputItems: [
              {
                ...
                sourceUrl: "https://example.test/generated-source.png",
              },
            ],
          };
        },
    ...
    workbench.ui.selectedEpisodeId = "10000000-0000-4000-8000-000000000014";
    workbench.ui.canvasDocument.episodeId = "10000000-0000-4000-8000-000000000014";
  ...
  it("restores canvas credits and shows missing model key errors when sync task creation fails", async () => {
    const workbench = {
      state: buildProjectState(),
      api: {
        async createImageTask() {
          return {
            taskId: "task-canvas-key-missing",
            status: "failed",
            creditBalance: 500,
            failureCode: "provider_api_key_missing",
            failure: {
              failureCode: "provider_api_key_missing",
              displayMessage: "供应商 API 密钥缺失。",
    ...
    assert.equal(workbench.ui.canvasRunPreview ?? null, null);
    ...
    assert.match(workbench.ui.toast.message, /供应商 API 密钥缺失/);
    assert.match(workbench.ui.toast.message, /VOLCENGINE_ARK_API_KEY/);
  });
  ...
  it("submits standalone canvas runs through the canvas run API without pre-creating an episode", async () => {
    ...
    const workbench = {
      ...
      api: {
        ...
        async createImageTask(episodeId, payload) {
          createImageTaskCalls.push({ episodeId, payload });
          return { platform: { tasks: [{ taskId: "task-should-not-create" }] } };
        },
    ...
    assert.equal(createImageTaskCalls.length, 0);
    assert.equal(runCanvasNodeCalls.length, 1);
    assert.equal(runCanvasNodeCalls[0].canvasProjectId, "canvas-main");
    assert.equal(runCanvasNodeCalls[0].nodeId, "send-flow");
    assert.equal(runCanvasNodeCalls[0].payload.episodeId, undefined);
    assert.equal(runCanvasNodeCalls[0].payload.prompt, "生成画面");
    assert.equal(runCanvasNodeCalls[0].payload.kind, "image");
    assert.equal(workbench.ui.canvasRunPreview.taskId, "task-canvas-run-api");
    assert.equal(workbench.ui.toast, "");
  });
  ...
  it("uses the saved standalone canvas id when submitting a canvas run", async () => {
    ...
    const canvasDocument = {
      version: 1,
      canvasProjectId: "local-stale-canvas",
      projectId: "local-stale-canvas",
      episodeId: "",
      viewport: { x: 0, y: 0, zoom: 1 },
      nodes: [
        {
    ...
    assert.equal(runCanvasNodeCalls.length, 1);
    assert.equal(runCanvasNodeCalls[0].canvasProjectId, "server-canvas-main");
    assert.equal(runCanvasNodeCalls[0].nodeId, "send-flow");
  ...
  it("submits canvas image runs with empty prompt when connected text and images are present", async () => {
    ...
    assert.equal(createImageTaskCalls.length, 1);
    assert.equal(createImageTaskCalls[0].payload.prompt, "第一章：主角进入废墟。");
      ...
      const workbench = {
        ...
        ui: buildProjectUi({
          ...
          canvasDocument: {
            version: 1,
            projectId: "canvas-project-main",
            episodeId: "10000000-0000-4000-8000-000000000005",
            viewport: { x: 0, y: 0, zoom: 1 },
            nodes: [
              {
                id: "upload-first",
  ...
  it("does not use a grandparent upload as a canvas video reference before the image node has generated", async () => {
    ...
    const canvasDocument = {
      ...
      edges: [
        { id: "edge-upload-image", sourceNodeId: "uploaded-source", sourcePortId: "out_image", targetNodeId: "pending-image", targetPortId: "in_image", data: { kind: "image" } },
        { id: "edge-image-video", sourceNodeId: "pending-image", sourcePortId: "out_image", targetNodeId: "video-send", targetPortId: "in_image", data: { kind: "image" } },
      ],
    };
    const workbench = {
      state: buildProjectState(),
      api: {
        async createVideoTask(episodeId, payload) {
          createVideoTaskCalls.push({ episodeId, payload });
          return { platform: { tasks: [{ taskId: "task-canvas-video-no-grandparent" }] } };
        },
      },
      ui: buildProjectUi({
        activeNavTab: "tools",
  ...
  it("uses a connected canvas image node as a video reference after that image has generated", async () => {
    ...
    const canvasDocument = {
      ...
      edges: [
        { id: "edge-image-video", sourceNodeId: "generated-image", sourcePortId: "out_image", targetNodeId: "video-send", targetPortId: "in_image", data: { kind: "image" } },
      ],
    };
    const workbench = {
      state: buildProjectState(),
      api: {
        async createVideoTask(episodeId, payload) {
          createVideoTaskCalls.push({ episodeId, payload });
          return { platform: { tasks: [{ taskId: "task-canvas-video-image-result" }] } };
        },
      },
      ui: buildProjectUi({
        activeNavTab: "tools",
  ...
  it("blocks canvas video generation when connected image aspect ratio is outside Seedance limits", async () => {
    const createVideoTaskCalls = [];
    const workbench = {
      state: buildProjectState(),
      api: {
      ...
      ui: buildProjectUi({
        activeNavTab: "tools",
        selectedEpisodeId: "10000000-0000-4000-8000-000000000006",
        selectedCanvasNodeId: "video-send",
        canvasProjectView: "detail",
        creditBalance: 99999,
        episodeGenerationConfig: {
        ...
        canvasDocument: {
          version: 1,
          projectId: "canvas-project-main",
          episodeId: "10000000-0000-4000-8000-000000000006",
          viewport: { x: 0, y: 0, zoom: 1 },
          nodes: [
            {
              id: "wide-upload",
              ...
              data: {
                ...
                previewUrl: "https://example.test/too-wide.png",
                width: 2990,
                height: 1000,
                ports: { outputs: [{ id: "out_image", kind: "image" }] },
              },
          ...
          edges: [
            { id: "edge-wide-video", sourceNodeId: "wide-upload", sourcePortId: "out_image", targetNodeId: "video-send", targetPortId: "in_image", data: { kind: "image" } },
          ],
        },
      }),
      root: { innerHTML: "", querySelector() { return null; } },
  ...
  it("reuses loaded asset conversation history when toggling back to assets scope", async () => {
    ...
    try {
      await handleWorkbenchActionForTest(workbench, {
        dataset: { action: "set-muse-scope-mode", mode: "storyboard" },
      });
      await Promise.resolve();
      await Promise.resolve();
  ...
  it("clears stale episode asset delete toast when opening and closing manual asset creation", async () => {
    ...
    await handleWorkbenchActionForTest(workbench, {
      dataset: { action: "open-episode-asset-create-modal" },
    });
  ...
  it("keeps existing episode storyboards when add storyboard falls back after episode_not_found", async () => {
    ...
    assert.notEqual(storyboards[1].id, existingStoryboard.id);
  });
  ...
  it("normalizes persisted other asset tab in the episode asset toolbar", () => {
    ...
    assert.match(html, /data-action="open-asset-import-modal" data-asset-kind="character"/);
    assert.match(html, /data-action="open-episode-team-asset-library" data-asset-kind="character"/);
    ...
    const payload = buildImageGenerationPayload({
      ...
      ui: {
        ...
        importedAssets: {
          character: [
            {
              id: "character-drag-send-1",
              name: "拖入角色",
              description: "深色工装，短发，正面站姿。",
              previewUrl: "https://example.com/drag-send-character.png",
            },
          ],
          scene: [],
          prop: [],
        ...
        assetPromptDraft: {
          ...
          quickReferenceItems: [
            {
              id: "drag-ref:asset-reference:character-drag-send-1",
              assetId: "character-drag-send-1",
              kind: "image",
              type: "image",
              name: "拖入角色",
              preview: "https://example.com/drag-send-character.png",
              url: "https://example.com/drag-send-character.png",
              fromQuickReference: true,
            },
          ],
        },
  ...
  it("shows project assets inside the episode workbench import modal", () => {
      ...
      ui: {
        ...
        episodeWorkbenchContext: {
          assetsByType: {
            ...
            scene: [
              {
                id: "episode-scene-context",
                name: "项目内场景",
                previewUrl: "data:image/svg+xml;charset=UTF-8,project-scene-context",
              },
            ],
            prop: [],
          },
    ...
    assert.match(html, /data-asset-id="episode-scene-context"/);
    assert.match(html, /项目内场景/);
    assert.match(html, /project-scene-context/);
  ...
  it("imports a selected episode asset into the matching tab and refreshes episode assets from backend", async () => {
    ...
    assert.equal(workbench.ui.selectedEpisodeCardId, "imported-scene-1");
    ...
    const workbench = {
      ...
      api: {
        createAiStoryboardPreviewStream: async function* () {
          ...
          yield { event: "asset_done", data: { stage: "scene", title: "场景提示词生成", text: '{"scenes":[{"sceneName":"闵婶家门前","sceneDescription":"旧木屋门前"}]}' } };
          yield { event: "asset_done", data: { stage: "character", title: "角色提示词生成", text: '{"characters":[{"characterName":"任小野","characterDescription":"旧布短衣"}]}' } };
          yield { event: "asset_done", data: { stage: "prop", title: "道具提示词生成", text: '{"props":[{"propName":"饭食","propDescription":"旧布包"}]}' } };
          yield { event: "asset_done", data: { stage: "shot", title: "分镜提示词生成", text: '{"storyboards":[{"shotNo":1,"plot":"递出饭食","dialogue":"麻烦您了","imagePrompt":"任小野递出饭食","videoPrompt":"中景固定镜头"}]}' } };
        },
      },
      ui: {
        ...buildProjectUi({
                ...
                rows: [
                  {
                    plot: "叶焚野盯着屏幕弹窗。",
                    dialogue: "无台词，内心OS",
                    imagePrompt: "出租屋内，屏幕弹窗。",
                    videoPrompt: "键盘敲击，屏幕弹窗。",
                  },
                ],
              },
            },
...
describe("storyboard state", () => {
  it("renders single-episode naming modal and sorts custom episodes newest first", () => {
    ...
    assert.match(modalHtml, /AI 智能分镜 20积分/);
  ...
  it("refreshes script model credits every time single-episode creation opens", async () => {
    ...
    assert.deepEqual(configCalls, [{ fresh: true }]);
    assert.match(workbench.root.innerHTML, /AI 智能分镜 35积分/);
    assert.doesNotMatch(workbench.root.innerHTML, /AI 智能分镜 200积分/);
  });
