...
it("keeps the scissors control under the pointer on a hovered X6 edge and removes only that edge", () => {
  ...
  assert.match(bindSource, /graph\.clientToLocal\?\.\(clientX, clientY\)/);
  assert.match(bindSource, /getClosestPointRatio\?\.\(localPoint\)/);
  assert.match(bindSource, /activeTool\.options\.distance = distance/);
  assert.match(bindSource, /className: "canvas-edge-disconnect-tool"/);
  assert.match(bindSource, /const queueShow = \(request, \{ immediate = false \} = \{\}\) =>/);
  assert.match(bindSource, /EDGE_DISCONNECT_SHOW_DELAY_MS = 500/);
  assert.match(bindSource, /showTimer = globalThis\.setTimeout\?\.\(\(\) => \{[\s\S]*?\}, EDGE_DISCONNECT_SHOW_DELAY_MS\)/);
  ...
  assert.doesNotMatch(bindSource, /}, 1000\)/);
  assert.match(bindSource, /graph\.on\("edge:click", \(event\) => showFromGraphEvent\(event, \{ immediate: true \}\)\)/);
  assert.match(bindSource, /button\.style\.display = "grid"/);
  assert.match(bindSource, /const scheduleHide = \(\) => \{[\s\S]*?hideTimer = globalThis\.setTimeout\?\.\(hide, 400\)/);
  assert.match(bindSource, /mount\.append\(button\)/);
  assert.match(bindSource, /button\.style\.left = `\$\{left\}px`/);
  assert.match(bindSource, /button\.style\.top = `\$\{top\}px`/);
  ...
  assert.match(bindSource, /graph\.on\("cell:mouseenter", showFromGraphEvent\)/);
  ...
  assert.match(bindSource, /findEdgeViewsFromPoint/);
  ...
  assert.doesNotMatch(bindSource, /distance: "50%"/);
  assert.match(bindSource, /"canvas-edge-disconnect", \{ local: true, reset: true \}/);
  assert.match(bindSource, /onClick\(\{ cell \}\)[\s\S]*?disconnectEdge\(cell\)/);
  assert.match(source, /"vector-effect": "non-scaling-stroke"/);
  assert.match(source, /graph\.on\("cell:click", \(\{ cell, e \}\) => \{[\s\S]*?isCanvasGraphEdge\(cell\)/);
  ...
  assert.match(styles, /\.canvas-edge-disconnect-tool\s*\{[\s\S]*?animation:\s*canvas-edge-disconnect-reveal 420ms ease-out forwards/);
