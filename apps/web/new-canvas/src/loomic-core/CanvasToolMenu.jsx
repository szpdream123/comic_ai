...
import { CANVAS_SHORTCUT_GROUPS, matchesCanvasShortcut } from "./canvas-shortcuts.js";
...
export function CanvasToolMenu({
  ...
  useEffect(() => {
    const handlePointerDown = (event) => {
      if (!toolMenuRef.current?.contains(event.target) && !toolboxRef.current?.contains(event.target)) closeToolPopovers();
    };
  ...
  }, [closeToolPopovers, toggleAddMenu]);
  ...
  const createTextNode = useCallback(() => {
    closeToolPopovers();
    const id = createTextNodeElement(excalidrawApi, brandKitTextNodeOptions(brandKit));
  ...
  const createImageGenerator = useCallback(() => {
    closeToolPopovers();
    const id = createImageGeneratorElement(excalidrawApi);
    excalidrawApi.updateScene({ appState: { selectedElementIds: { [id]: true } } });
  }, [closeToolPopovers, excalidrawApi]);
  const createVideoGenerator = useCallback(() => {
    closeToolPopovers();
    const id = createVideoGeneratorElement(excalidrawApi, { aspectRatio: "16:9" });
    excalidrawApi.updateScene({ appState: { selectedElementIds: { [id]: true } } });
  }, [closeToolPopovers, excalidrawApi]);
  const createWorkflowNode = useCallback((type) => {
    closeToolPopovers();
    const id = createWorkflowNodeElement(excalidrawApi, type);
  ...
  const importMediaFiles = useCallback(async (event) => {
    ...
    for (const file of files) {
      if (onImportImage) await onImportImage(file, excalidrawApi);
      else if (file.type.startsWith("image/")) await insertImageFileOnCanvas(excalidrawApi, file);
    }
    event.target.value = "";
  ...
  return (
    <>
      <div
        ...
        style={{ left: leftPanelOpen ? "calc(50% + 140px)" : "50%" }}
        role="toolbar"
        aria-label="画布工具"
      ...
      >
        <div className="loomic-tool-popover-anchor">
          ...
          {addMenuOpen ? (
            <div className="loomic-tool-popover loomic-add-node-menu" role="menu" aria-label="添加节点">
              ...
              <button type="button" role="menuitem" onClick={createTextNode}>
                ...
                <span><strong>文本</strong><small>添加提示词或创作说明</small></span>
              </button>
              <button type="button" role="menuitem" onClick={createImageGenerator}>
                ...
                <span><strong>图片生成</strong><small>创建图片生成节点</small></span>
              </button>
              <button type="button" role="menuitem" onClick={createVideoGenerator}>
                ...
                <span><strong>视频生成</strong><small>创建视频生成节点</small></span>
              </button>
              <button type="button" role="menuitem" onClick={() => createWorkflowNode("video-composition-node")}>
                ...
                <span><strong>视频合成</strong><small>连接已归档片段并输出 MP4</small></span>
              </button>
              <button type="button" role="menuitem" onClick={() => createWorkflowNode("director-node")}>
                ...
                <span><strong>导演台</strong><small>连接导演指令或打开独立工作区</small></span>
              </button>
              <button type="button" role="menuitem" onClick={() => createWorkflowNode("audio-node")}>
                ...
                <span><strong>音频</strong><small>文本转语音，需后台音频模型</small></span>
              </button>
              <button type="button" role="menuitem" onClick={() => createWorkflowNode("script-node")}>
                ...
                <span><strong>脚本</strong><small>添加可连接的脚本文本源</small></span>
              </button>
              <button type="button" role="menuitem" onClick={openImagePicker}>
                ...
                <span><strong>上传素材</strong><small>从本机导入图片、视频或音频</small></span>
              </button>
            </div>
          ) : null}
        </div>
        ...
        <span className="loomic-tool-separator" />
        <div className="loomic-tool-popover-anchor">
          <button
            type="button"
          ...
          {movementMenuOpen ? (
            <div className="loomic-tool-popover loomic-movement-menu" role="menu" aria-label="移动工具">
              {MOVEMENT_TOOLS.map((tool) => {
                const Icon = tool.Icon;
                return (
                  <button key={tool.type} type="button" role="menuitemradio" aria-checked={activeTool === tool.type} aria-keyshortcuts={tool.shortcut} className={activeTool === tool.type ? "is-active" : ""} onClick={() => setTool(tool.type)}>
        ...
        {onOpenFilesView ? (
          <>
            <button type="button" className="loomic-tool-button" title="素材库" aria-label="素材库" onClick={() => openFilesView("assets")}><Images aria-hidden="true" /></button>
        ...
        {shortcutsOpen ? (
          <section className="loomic-tool-popover loomic-shortcuts-panel" role="dialog" aria-label="快捷键">
            <header>
              <div><strong>快捷键</strong><span>快速控制画布</span></div>
              <button type="button" className="loomic-icon-button" aria-label="关闭快捷键" onClick={() => setShortcutsOpen(false)}><X aria-hidden="true" /></button>
            </header>
      ...
      {templateMenuOpen ? (
        <section ref={toolboxRef} className="loomic-tool-popover loomic-template-menu" role="dialog" aria-label="工具箱">
          <header className="loomic-toolbox-header">
            <div className="loomic-toolbox-categories" role="tablist" aria-label="工具分类">
              {CANVAS_TOOL_PRESET_CATEGORIES.map((category) => (
                <button key={category.id} type="button" role="tab" aria-selected={toolboxCategory === category.id} className={toolboxCategory === category.id ? "is-active" : ""} onClick={() => setToolboxCategory(category.id)}>{category.id === "all" ? "我的工具箱" : category.label}</button>
              ))}
            </div>
            <button
              type="button"
          ...
          <div className="loomic-toolbox-state loomic-toolbox-intro">
            <span>使用工具箱模板加速创作，快速构建你的专属工具箱。</span>
            <a href={CREATOR_GUIDE_URL} target="_blank" rel="noopener noreferrer">查看教程</a>
          </div>
