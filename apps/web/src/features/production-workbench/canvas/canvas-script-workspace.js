...
function workspaceShotRow(node = {}, index, selected = false) {
  ...
  return `<tr data-script-workspace-child="${escapeAttr(nodeId)}">
    ...
    <td><button type="button" data-action="run-canvas-node" data-node-id="${escapeAttr(nodeId)}">生成视频</button><button type="button" data-action="delete-canvas-node" data-node-id="${escapeAttr(nodeId)}" aria-label="删除镜头">删除</button></td>
  </tr>`;
}
...
export function renderCanvasScriptWorkspace(ui = {}) {
  ...
  return `<section class="script-workspace-layer" data-script-workspace-layer aria-label="脚本节点工作区">
    ...
    <section class="script-workspace" role="dialog" aria-modal="true" aria-labelledby="script-workspace-title">
      <header class="script-workspace-header"><div><small>脚本分镜</small><h2 id="script-workspace-title">${escapeHtml(scriptNode.data?.title === "剧本工作流" ? "脚本分镜" : scriptNode.data?.title ?? "脚本分镜")}</h2></div><div class="script-workspace-header-actions"><button type="button" data-action="run-canvas-script-workflow" data-node-id="${escapeAttr(scriptNode.id)}" ${children.length ? "" : "disabled"}>依次执行</button><button type="button" data-action="download-canvas-selection" data-node-ids="${escapeAttr(JSON.stringify(shots.map((node) => node.id)))}" ${shots.length ? "" : "disabled"}>下载视频</button><button type="button" data-action="close-canvas-script-workspace" aria-label="关闭脚本分镜工作区">关闭</button></div></header>
