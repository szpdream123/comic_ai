import * as THREE from "/vendor/three.module.js?liquid-ether=3";
...
export function mountLiquidEther(mount, options = {}) {
  ...
  const manager = createLiquidEther(container, {
    ...DEFAULT_OPTIONS,
    ...options,
  });
...
function createLiquidEther(container, options) {
  ...
  class MouseClass {
    ...
    init(target) {
      this.container = target;
      this.docTarget = target.ownerDocument || null;
      const defaultView =
  ...
  class AutoDriver {
    ...
    pickNewTarget() {
      ...
      this.target.set((random() * 2 - 1) * (1 - this.margin), (random() * 2 - 1) * (1 - this.margin));
    }
...
`;
  ...
  class ExternalForce extends ShaderPass {
    ...
    dispose() {
      if (this.mouse) {
        this.mouse.geometry.dispose();
        this.mouse.material.dispose();
      }
    }
  }
  ...
  class Simulation {
    ...
    dispose() {
      for (const pass of [this.advection, this.externalForce, this.viscous, this.divergence, this.poisson, this.pressure]) {
  ...
  class WebGLManager {
    ...
    dispose() {
      try {
        ...
        // The hero should never block the rest of the workbench during teardown.
      }
    }
  }
