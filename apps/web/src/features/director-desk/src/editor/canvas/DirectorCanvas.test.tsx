...
it("disables orbit controls and marks the viewport while drawing a character route", () => {
  ...
  expect(screen.getByTestId("director-canvas")).toHaveClass("is-drawing-route");
...
it("updates the first-person render camera when the active camera changes", () => {
  const state = useDirectorStore.getState();
  const firstCamera = state.project.cameras[0]!;
  const secondCamera: typeof firstCamera = {
    ...firstCamera,
    id: "cam_2",
    name: "机位02",
    transform: { ...firstCamera.transform, position: [7, 3, 4] },
    target: [2, 1.5, -1],
    fov: 35,
  };
  const secondCameraSnapshot = getCameraViewSnapshotFromShot(secondCamera);
  useDirectorStore.setState({
    ...state,
    viewMode: "camera",
    project: {
      ...
      scene: { ...state.project.scene, pathCollisionEnabled: false },
      cameras: [
        {
          ...firstCamera,
          transform: { ...firstCamera.transform, position: [1, 2, 3] },
          target: [0, 1, 0],
          fov: 55,
        },
        secondCamera,
  ...
  act(() => useDirectorStore.getState().setActiveCamera("cam_2"));

  expect(mockCameraPositionSet).toHaveBeenCalledWith(...secondCameraSnapshot.position);
  expect(mockCameraLookAt).toHaveBeenCalledWith(...secondCameraSnapshot.target);
  expect(mockCameraUpdateProjectionMatrix).toHaveBeenCalled();
});
