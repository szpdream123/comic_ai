...
it("switches the active transform control mode from the viewport capsule", async () => {
  ...
  expect(screen.getByRole("button", { name: "姿势编辑" })).toHaveAttribute("aria-pressed", "false");
  ...
  await user.click(screen.getByRole("button", { name: "姿势编辑" }));
  expect(useDirectorStore.getState().transformMode).toBe("pose");
  expect(screen.getByRole("button", { name: "缩放" })).toHaveAttribute("aria-pressed", "false");
  expect(screen.getByRole("button", { name: "姿势编辑" })).toHaveAttribute("aria-pressed", "true");
  expect(screen.getByRole("button", { name: "姿势编辑" })).toHaveClass("is-active");
...
it("selects the default mannequin when an initial transform tool is clicked", async () => {
  const user = userEvent.setup();
  render(<ViewportToolbar />);
...
it("adds a character when the toolbar is mounted inside a shadow root", async () => {
  const user = userEvent.setup();
  ...
  const view = render(<ViewportToolbar />, { container });
  const shadowScreen = within(container);

  await user.click(shadowScreen.getByRole("button", { name: "添加角色" }));
  await user.click(shadowScreen.getByRole("menuitem", { name: "男性素体" }));
