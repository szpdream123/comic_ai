...
import {
  getPoseJointControlPatch,
  getPoseJointRotation,
  POSE_JOINT_CONNECTIONS,
  POSE_JOINT_DEFINITIONS,
  type PoseJointId,
} from "../schema/poseHandles";
...
import { getUE4GroundedLabelY, UE4_MANNEQUIN_BONE_MAP } from "../runtime/ue4Mannequin/ue4MannequinRig";
...
const CHARACTER_ROUTE_DRAW_MIN_DISTANCE = 0.45;
...
function ViewportObjectLabel({
  ...
  return (
    <Html
      ...
      sprite
      transform
      wrapperClass="viewport-object-label-wrapper"
      zIndexRange={[0, 1]}
    >
...
const PROCEDURAL_POSE_ANCHOR_NAMES: Record<PoseJointId, string> = {
  body: "humanoid-pelvis",
  torso: "humanoid-chest",
  head: "humanoid-head",
...
const MIXAMO_POSE_ANCHOR_NAMES: Record<PoseJointId, string> = {
  body: "mixamorig:Hips",
...
function getPoseAnchorName(item: DirectorObject, jointId: PoseJointId) {
  if (item.characterRig?.rigType === "ue4-mannequin") {
    return UE4_MANNEQUIN_BONE_MAP[jointId];
  }
  if (item.characterRig?.rigType === "mixamo") {
    return MIXAMO_POSE_ANCHOR_NAMES[jointId];
  }
  return PROCEDURAL_POSE_ANCHOR_NAMES[jointId];
}

function createPoseVectorMap() {
  return Object.fromEntries(POSE_JOINT_DEFINITIONS.map(({ id }) => [id, new Vector3()])) as Record<PoseJointId, Vector3>;
}

function readPoseThemeColors() {
  const fallback = { accent: "#0396FF", selected: "#FFFFFF" };
  if (typeof window === "undefined") return fallback;

  const styles = window.getComputedStyle(getDirectorDeskThemeRoot());
  const readRgb = (property: string, fallbackColor: string) => {
    const channels = styles.getPropertyValue(property).match(/[\d.]+/g)?.slice(0, 3).map(Number);
  ...
  return {
    accent: readRgb("--accent-rgb", fallback.accent),
    selected: readRgb("--text-rgb", fallback.selected),
  };
}

function CharacterPoseRig({ characterRoot, item }: { characterRoot: React.RefObject<Group>; item: DirectorObject }) {
  const [selectedJointId, setSelectedJointId] = useState<PoseJointId | null>(null);
  const [poseColors, setPoseColors] = useState(readPoseThemeColors);
  const handleRefs = useRef<Partial<Record<PoseJointId, Mesh>>>({});
  const pivotRef = useRef<Group>(null!);
  const draggingRef = useRef(false);
  const poseColorsRef = useRef(poseColors);
  const lastThemeCheckRef = useRef(-Infinity);
  const anchorPositionsRef = useRef(createPoseVectorMap());
  const visibleAnchorIdsRef = useRef(new Set<PoseJointId>());
  const parentInverse = useMemo(() => new Matrix4(), []);
  const worldPosition = useMemo(() => new Vector3(), []);
  const parentScale = useMemo(() => new Vector3(), []);
  const lineGeometry = useMemo(() => {
    const geometry = new BufferGeometry();
    geometry.setAttribute("position", new BufferAttribute(new Float32Array(POSE_JOINT_CONNECTIONS.length * 6), 3));
    geometry.setDrawRange(0, 0);
    return geometry;
  }, []);
  const updatePoseControls = useDirectorStore((state) => state.updatePoseControls);
  const controls = item.characterRig?.controls ?? {};
  const selectedJoint = POSE_JOINT_DEFINITIONS.find(({ id }) => id === selectedJointId) ?? null;

  useEffect(() => () => lineGeometry.dispose(), [lineGeometry]);

  useLayoutEffect(() => {
    if (!selectedJointId || !pivotRef.current?.rotation?.set || draggingRef.current) return;
    pivotRef.current.rotation.set(...getPoseJointRotation(selectedJointId, controls));
  }, [controls, selectedJointId]);

  useFrame(({ camera, clock }) => {
    if (clock.elapsedTime - lastThemeCheckRef.current >= 0.2) {
      lastThemeCheckRef.current = clock.elapsedTime;
      const nextColors = readPoseThemeColors();
      if (
        nextColors.accent !== poseColorsRef.current.accent ||
        nextColors.selected !== poseColorsRef.current.selected
      ) {
        poseColorsRef.current = nextColors;
        setPoseColors(nextColors);
      }
    }

    const root = characterRoot.current;
    ...
    visibleAnchorIdsRef.current.clear();

    POSE_JOINT_DEFINITIONS.forEach(({ id }) => {
      const handle = handleRefs.current[id];
      const anchor = root.getObjectByName(getPoseAnchorName(item, id));
      if (!handle || !anchor) {
      ...
      const handleSize = Math.min(0.11, Math.max(0.045, camera.position.distanceTo(worldPosition) * 0.012));
      const localPosition = anchorPositionsRef.current[id].copy(worldPosition).applyMatrix4(parentInverse);
      const parentScaleFactor = Math.max(0.0001, (Math.abs(parentScale.x) + Math.abs(parentScale.y) + Math.abs(parentScale.z)) / 3);

      handle.visible = true;
      handle.position.copy(localPosition);
      handle.scale.setScalar((handleSize / parentScaleFactor) * (id === selectedJointId ? 1.28 : 1));
      visibleAnchorIdsRef.current.add(id);
    });

    const positionAttribute = lineGeometry.getAttribute("position") as BufferAttribute;
    const positions = positionAttribute.array as Float32Array;
    let offset = 0;
    POSE_JOINT_CONNECTIONS.forEach(([startId, endId]) => {
      if (!visibleAnchorIdsRef.current.has(startId) || !visibleAnchorIdsRef.current.has(endId)) return;
      const start = anchorPositionsRef.current[startId];
      const end = anchorPositionsRef.current[endId];
    ...
    lineGeometry.setDrawRange(0, offset / 3);
    positionAttribute.needsUpdate = true;

    if (pivotRef.current && selectedJointId && visibleAnchorIdsRef.current.has(selectedJointId)) {
      pivotRef.current.visible = true;
      pivotRef.current.position.copy(anchorPositionsRef.current[selectedJointId]);
  ...
  const commitJointRotation = useCallback(() => {
    if (!selectedJointId || !pivotRef.current?.rotation) return;
    updatePoseControls(
      item.id,
      getPoseJointControlPatch(selectedJointId, [
        pivotRef.current.rotation.x,
        pivotRef.current.rotation.y,
        pivotRef.current.rotation.z,
      ])
    );
  }, [item.id, selectedJointId, updatePoseControls]);

  return (
    <group name="character-pose-rig" userData={{ [HIDE_FROM_VIEWPORT_CAPTURE_KEY]: true }}>
      <lineSegments geometry={lineGeometry} renderOrder={998}>
        <lineBasicMaterial color={poseColors.accent} depthTest={false} opacity={0.64} transparent />
      </lineSegments>
      {POSE_JOINT_DEFINITIONS.map(({ id, label }) => {
        const selected = selectedJointId === id;
        return (
          <mesh
            key={id}
            ref={(mesh) => {
              handleRefs.current[id] = mesh ?? undefined;
            }}
            name={`pose-joint-${id}`}
            renderOrder={999}
            visible={false}
            userData={{ poseJointId: id, poseJointLabel: label, [HIDE_FROM_VIEWPORT_CAPTURE_KEY]: true }}
          ...
          >
            ...
            <meshBasicMaterial
              color={selected ? poseColors.selected : poseColors.accent}
              depthTest={false}
      ...
      <group ref={pivotRef} name="pose-joint-pivot">
        {selectedJoint ? (
          <Html
            center
            pointerEvents="none"
            ...
            zIndexRange={[1, 2]}
          >
            <div className="pose-joint-label">{selectedJoint.label}</div>
          </Html>
        ) : null}
      </group>
      {selectedJointId ? (
        <ViewportTransformControls
          ...
          onObjectChange={commitJointRotation}
          showX
          showY={!selectedJoint?.hinge}
          showZ={!selectedJoint?.hinge}
          size={0.72}
          space="local"
        />
      ) : null}
...
function ObjectSceneNode({
  ...
  const [measuredCharacterWorldLabel, setMeasuredCharacterWorldLabel] = useState<{
    key: string;
    position: [number, number, number];
  } | null>(null);
  ...
  const fallbackCharacterLabelPosition: [number, number, number] = [
    item.transform.position[0],
    item.transform.position[1] + characterLabelY * Math.abs(item.transform.scale[1]),
    item.transform.position[2],
  ];
  const characterLabelPosition =
    measuredCharacterWorldLabel?.key === characterLabelKey
      ? measuredCharacterWorldLabel.position
      : fallbackCharacterLabelPosition;
  ...
  useLayoutEffect(() => {
    ...
    if (item.kind !== "character" || !showLabels || !group?.isObject3D) return;
    ...
    const nextPosition: [number, number, number] = [
      ...
      Number((bounds.max.y + VIEWPORT_OBJECT_LABEL_VERTICAL_GAP).toFixed(4)),
      Number(center.z.toFixed(4)),
    ];

    setMeasuredCharacterWorldLabel((current) => {
      if (
        current?.key === characterLabelKey &&
        current.position.every((value, index) => Math.abs(value - nextPosition[index]) < 0.0001)
      ) {
        return current;
      }

      return { key: characterLabelKey, position: nextPosition };
    });
  }, [animatedCharacterRig, characterLabelKey, characterLabelY, item.kind, item.transform, showLabels]);

  function commitTransformFromViewport() {
    const group = groupRef.current;
    if (!group) return;

    updateObjectDisplayTransform(item.id, {
  ...
  return (
    <>
      {node}
      {characterLabel}
      {transformMode === "pose" && item.kind === "character" && item.characterRig ? (
        <CharacterPoseRig characterRoot={groupRef} item={item} />
      ) : null}
      <ViewportTransformControls
        mode={transformMode}
        object={groupRef}
...
function CharacterRoutePointHandle({
  ...
  const node = (
    ...
    >
      ...
      {showLabel ? (
        <ViewportObjectLabel position={[0, compact ? 0.25 : 0.34, 0]}>
...
function CharacterRouteRig({
  ...
  const routePoints = useMemo(
    () => {
      const positions = path.interpolation === "linear"
        ? path.keyframes.map((keyframe) =>
            constrainObjectMotionTransform(character, keyframe.transform, scene, objects).position
          )
      ...
      return positions.map((position) => [position[0], position[1] + 0.035, position[2]] as [number, number, number]);
    },
    [character, objects, path.interpolation, path.keyframes, scene]
  );
  const compactHandles = path.keyframes.length > 12;
    ...
    <group userData={{ [HIDE_FROM_VIEWPORT_CAPTURE_KEY]: true }}>
      ...
      {showHandles ? path.keyframes.map((keyframe, index) => (
        <CharacterRoutePointHandle
          ...
          showLabel={
            !compactHandles
            || selectedObjectMotionKeyframeId === keyframe.id
            || index === 0
            || index === path.keyframes.length - 1
          }
          transformMode={transformMode}
          translationSnap={translationSnap}
        />
...
export function SceneRoot({ renderMode = "interactive" }: { renderMode?: SceneRootRenderMode }) {
  ...
  const draftRoutePointsRef = useRef<Array<[number, number, number]>>([]);
  const [draftRoutePoints, setDraftRoutePoints] = useState<Array<[number, number, number]>>([]);
  ...
  const replaceCharacterRouteFromPoints = useDirectorStore((state) => state.replaceCharacterRouteFromPoints);
  const selectObject = useDirectorStore((state) => state.selectObject);
  ...
  const drawingCharacter = objects.find(
    (item) => item.id === characterRouteDrawingObjectId && item.kind === "character"
  );

  useEffect(() => {
    draftRoutePointsRef.current = [];
    setDraftRoutePoints([]);
  }, [characterRouteDrawingObjectId]);

  function getLocalRoutePoint(event: ThreeEvent<PointerEvent>): [number, number, number] {
    const eventPoint = event.point ?? (event.nativeEvent as PointerEvent & { point?: Vector3 }).point;
    const point = eventPoint?.clone() ?? new Vector3();
    const root = sceneRootRef.current;
    if (root && typeof root.worldToLocal === "function") root.worldToLocal(point);
    return [point.x, scene.groundHeight, point.z];
  }

  function appendDraftRoutePoint(
    points: Array<[number, number, number]>,
    point: [number, number, number],
    minimumDistance: number
  ) {
    const previous = points[points.length - 1];
    if (previous && Math.hypot(point[0] - previous[0], point[2] - previous[2]) < minimumDistance) {
      return points;
    }
    return [...points, point];
  }

  function handleRouteDrawStart(event: ThreeEvent<PointerEvent>) {
    if (!drawingCharacter) return;
    event.stopPropagation();
    const displayedTransform = getObjectMotionSnapshot(drawingCharacter, cameraMotionProgress);
    let points: Array<[number, number, number]> = [[
      displayedTransform.position[0],
      scene.groundHeight,
      displayedTransform.position[2],
    ]];
    points = appendDraftRoutePoint(points, getLocalRoutePoint(event), 0.001);
    draftRoutePointsRef.current = points;
    setDraftRoutePoints(points);
  }

  function handleRouteDrawMove(event: ThreeEvent<PointerEvent>) {
    if (!drawingCharacter || draftRoutePointsRef.current.length === 0) return;
    event.stopPropagation();
    const points = appendDraftRoutePoint(
      draftRoutePointsRef.current,
      getLocalRoutePoint(event),
      CHARACTER_ROUTE_DRAW_MIN_DISTANCE
    );
    if (points === draftRoutePointsRef.current) return;
    draftRoutePointsRef.current = points;
    setDraftRoutePoints(points);
  }

  function handleRouteDrawEnd(event: ThreeEvent<PointerEvent>) {
    if (!drawingCharacter || draftRoutePointsRef.current.length === 0) return;
    event.stopPropagation();
    const points = appendDraftRoutePoint(draftRoutePointsRef.current, getLocalRoutePoint(event), 0.001);
    draftRoutePointsRef.current = points;
    setDraftRoutePoints(points);
    if (points.length >= 2) replaceCharacterRouteFromPoints(drawingCharacter.id, points);
  }
  ...
  return (
    ...
    >
      ...
      {interactive && drawingCharacter && viewMode === "director" ? (
        <>
          <mesh
            name="character-route-draw-surface"
            position={[0, scene.groundHeight + 0.002, 0]}
            rotation={[-Math.PI / 2, 0, 0]}
            onPointerDown={handleRouteDrawStart}
            onPointerMove={handleRouteDrawMove}
            onPointerUp={handleRouteDrawEnd}
          >
            <planeGeometry args={[200, 200]} />
            <meshBasicMaterial depthWrite={false} opacity={0} transparent />
          </mesh>
          {draftRoutePoints.length >= 2 ? (
            <>
              <Line
                ...
                opacity={0.82}
                points={draftRoutePoints.map((point) => [point[0], point[1] + 0.04, point[2]] as [number, number, number])}
                transparent
              />
              <Line
                color="#B8F8D0"
                ...
                opacity={1}
                points={draftRoutePoints.map((point) => [point[0], point[1] + 0.042, point[2]] as [number, number, number])}
                transparent
              />
            </>
          ) : null}
        ...
        .map((item) => {
          ...
          return (
            <ObjectSceneNode
              ...
              translationSnap={translationSnap}
              onSelect={characterRouteDrawingObjectId ? undefined : handleObjectSelect}
            />
          );
        })}
