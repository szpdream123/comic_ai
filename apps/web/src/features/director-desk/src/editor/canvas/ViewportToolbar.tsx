...
export function ViewportToolbar({
  ...
  useEffect(() => {
    ...
    function closeMenusOnOutsidePointerDown(event: PointerEvent) {
      ...
      if (toolbarRef.current && eventPath.includes(toolbarRef.current)) return;
      if (characterMenuRef.current && eventPath.includes(characterMenuRef.current)) return;
      if (geometryMenuRef.current && eventPath.includes(geometryMenuRef.current)) return;
      if (crowdPanelRef.current && eventPath.includes(crowdPanelRef.current)) return;
      if (modelLibraryPanelRef.current && eventPath.includes(modelLibraryPanelRef.current)) return;
  ...
  function selectTransformMode(mode: TransformMode) {
    ...
    if (!state.selectedObjectId && !state.selectedCrowdId) {
      const defaultCharacter = state.project.objects.find(
        (item) => item.kind === "character" && item.visible && !item.locked && !item.crowdId
      );
      if (defaultCharacter) state.selectObject(defaultCharacter.id);
    }
  ...
  const actions: ToolbarAction[] = [
    ...
    { label: "姿势编辑", icon: PersonStanding, mode: "pose", onClick: () => selectTransformMode("pose") },
    {
      label: "显示人物路线",
      icon: Route,
