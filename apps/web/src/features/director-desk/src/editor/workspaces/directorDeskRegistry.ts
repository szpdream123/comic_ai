...
const DIRECTOR_DESK_REGISTRY_KEY = "lingxi-3d-director-desk-registry-v1";
const ACTIVE_DIRECTOR_DESK_ID_KEY = "lingxi-3d-director-desk-active-id-v1";
const LEGACY_DIRECTOR_DESK_REGISTRY_KEY = "standalone-3d-director-desk-registry-v1";
const LEGACY_ACTIVE_DIRECTOR_DESK_ID_KEY = "standalone-3d-director-desk-active-id-v1";
...
export function readDirectorDeskRecords() {
  ...
  try {
    const raw = storage.getItem(DIRECTOR_DESK_REGISTRY_KEY) ?? storage.getItem(LEGACY_DIRECTOR_DESK_REGISTRY_KEY);
    if (!raw) return [];
...
export function readActiveDirectorDeskId() {
  ...
  try {
    const value = (
      storage.getItem(ACTIVE_DIRECTOR_DESK_ID_KEY)
      ?? storage.getItem(LEGACY_ACTIVE_DIRECTOR_DESK_ID_KEY)
    )?.trim();
    return value || null;
  } catch {
    return null;
...
function clearActiveDirectorDeskId() {
  ...
  try {
    ...
    storage.removeItem(LEGACY_ACTIVE_DIRECTOR_DESK_ID_KEY);
  } catch {
