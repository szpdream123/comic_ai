...
interface DirectorInternalState {
  ...
  characterRouteDrawingObjectId: string | null;
}
...
export interface DirectorActions {
  ...
  updateObjectDisplayTransform: (id: string, patch: Partial<DirectorTransform>) => void;
  addCharacterRoutePoint: (characterId: string) => string | null;
  replaceCharacterRouteFromPoints: (
    characterId: string,
    positions: Array<[number, number, number]>
  ) => void;
  setCharacterRouteDrawingObjectId: (objectId: string | null) => void;
...
function createStateFromSceneSnapshot(snapshot: unknown, options: DirectorStateOptions = {}): DirectorState | null {
  try {
    ...
    return {
      ...
      transformMode:
        state.transformMode === "rotate" || state.transformMode === "scale" || state.transformMode === "pose"
          ? state.transformMode
          : "translate",
...
export const useDirectorStore = create<DirectorStore>((set, get) => {
  ...
  return {
    ...
    setTransformMode: (mode) =>
      commitUiMutation((state) => ({
        ...
        selectedObjectMotionKeyframeId:
          mode === "pose" ? null : state.selectedObjectMotionKeyframeId,
      })),
    ...
    setCharacterRouteDrawingObjectId: (objectId) =>
      commitUiMutation((state) => ({
        ...state,
        characterRouteDrawingObjectId: objectId,
      })),
    ...
    updateObjectDisplayTransform: (id, patch) =>
      commitMutation((state) => {
        const currentObject = state.project.objects.find((item) => item.id === id);
        if (!currentObject) return state;

        const motionPath = normalizeObjectMotionPath(currentObject.motionPath, currentObject.transform);
        const currentDisplayTransform = getObjectMotionSnapshot(currentObject, state.cameraMotionProgress);
        const nextDisplayTransform: DirectorTransform = {
          position: patch.position ?? currentDisplayTransform.position,
        ...
        const rotationOffset: [number, number, number] = [
          ...
          nextDisplayTransform.rotation[1] - currentDisplayTransform.rotation[1],
        ...
        const applyDisplayDelta = (transform: DirectorTransform): DirectorTransform => ({
          position: roundTransformTuple([
            transform.position[0] + positionOffset[0],
            transform.position[1] + positionOffset[1],
            transform.position[2] + positionOffset[2],
          ]),
          rotation: roundTransformTuple([
            transform.rotation[0] + rotationOffset[0],
            transform.rotation[1] + rotationOffset[1],
            transform.rotation[2] + rotationOffset[2],
          ]),
          scale: roundTransformTuple([
        ...
        const nextObject: DirectorObject = motionPath.keyframes.length
          ? {
              ...currentObject,
              transform: applyDisplayDelta(currentObject.transform),
              motionPath: {
                ...motionPath,
                keyframes: motionPath.keyframes.map((keyframe) => ({
                  ...keyframe,
                  transform: applyDisplayDelta(keyframe.transform),
                })),
              },
            }
          : {
    ...
    replaceCharacterRouteFromPoints: (characterId, positions) =>
      commitMutation((state) => {
        const character = state.project.objects.find(
          (item) => item.id === characterId && item.kind === "character"
        );
        if (!character) return state;

        const cleanedPositions = positions.reduce<Array<[number, number, number]>>((result, position) => {
          if (position.length !== 3 || position.some((value) => !Number.isFinite(value))) return result;
          const nextPosition: [number, number, number] = [position[0], position[1], position[2]];
          const previous = result[result.length - 1];
          if (!previous || Math.hypot(
            nextPosition[0] - previous[0],
            nextPosition[1] - previous[1],
            nextPosition[2] - previous[2]
          ) > 0.001) {
            result.push(nextPosition);
          }
          return result;
        }, []);
        if (cleanedPositions.length < 2) return state;

        const segmentDistances = cleanedPositions.slice(1).map((position, index) => {
          const previous = cleanedPositions[index];
          return Math.hypot(
        ...
        const keyframes = cleanedPositions.map<DirectorObjectMotionKeyframe>((position, index) => {
          if (index > 0) traveledDistance += segmentDistances[index - 1];
          return {
            id: `${characterId}_motion_key_${index + 1}`,
            time: totalDistance > 0 ? traveledDistance / totalDistance : index / (cleanedPositions.length - 1),
            transform: {
              position,
              rotation: [...character.transform.rotation],
              scale: [...character.transform.scale],
            },
            actionPresetId: character.characterRig?.actionPresetId ?? null,
            facingMode: "path",
          };
        });
        const motionPath = normalizeObjectMotionPath(character.motionPath, character.transform);
        ...
        return {
          ...
          project: {
            ...
            objects: state.project.objects.map((item) =>
              item.id === characterId
                ? { ...item, motionPath: { ...motionPath, interpolation: "linear", keyframes } }
                : item
            ),
          },
        };
    ...
    deleteSelectedObject: () =>
      commitMutation((state) => {
        ...
        const nextFocusedCameras = nextCameras.map((camera) => {
          const clearCameraTarget = Boolean(
            camera.targetObjectId && selectedObjectIdSet.has(camera.targetObjectId)
          );
          const motionPath = camera.motionPath
            ? {
                ...camera.motionPath,
                keyframes: camera.motionPath.keyframes.map((keyframe) =>
                  keyframe.targetObjectId && selectedObjectIdSet.has(keyframe.targetObjectId)
                    ? { ...keyframe, targetMode: "manual" as const, targetObjectId: null }
                    : keyframe
                ),
              }
            : camera.motionPath;

          return clearCameraTarget || motionPath !== camera.motionPath
            ? {
                ...camera,
                targetMode: clearCameraTarget ? "manual" as const : camera.targetMode,
                targetObjectId: clearCameraTarget ? null : camera.targetObjectId,
                motionPath,
              }
            : camera;
        });
