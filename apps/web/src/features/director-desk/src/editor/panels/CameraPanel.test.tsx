...
it("switches camera shots when the panel is mounted inside the integrated shadow root", async () => {
  const user = userEvent.setup();
  ...
  const { unmount } = render(<CameraPanel />, { container });

  await user.click(view.getByLabelText("切换机位"));
  await user.click(within(view.getByRole("listbox", { name: "切换机位" })).getByRole("option", { name: "机位01" }));
