...
function formatAxisDisplayValue(value: FieldValue, precision?: number) {
  if (precision === undefined) return value;
  const parsed = parseFiniteNumber(value);
  return parsed === null ? value : parsed.toFixed(Math.min(Math.max(precision, 0), 6));
}
  ...
  return (
    <div className={`inspector-axis-input${isDragging ? " is-dragging" : ""}`}>
      ...
      <input
        ...
        step={control.step}
        type="number"
        value={isFocused ? control.value : formatAxisDisplayValue(control.value, control.displayPrecision)}
