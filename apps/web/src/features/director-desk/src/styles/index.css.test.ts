...
it("uses the StoryAI theme tokens instead of the temporary demo palette", () => {
  ...
  expect(css).toMatch(/:root\s*\{[\s\S]*?color-scheme:\s*light;[\s\S]*?--panel-rgb:\s*var\(--director-panel-rgb, 255 255 255\);[\s\S]*?--field-rgb:\s*var\(--director-field-rgb, 248 250 252\);[\s\S]*?--border-rgb:\s*var\(--director-border-rgb, 224 224 224\);[\s\S]*?--text-rgb:\s*var\(--director-text-rgb, 0 0 0\);/);
  expect(css).toMatch(/:root\[data-theme="dark"\],\s*[\r\n]+\s*:root\.dark\s*\{[\s\S]*?color-scheme:\s*dark;[\s\S]*?--panel-rgb:\s*var\(--director-panel-rgb, 26 26 26\);[\s\S]*?--field-rgb:\s*var\(--director-field-rgb, 10 10 12\);[\s\S]*?--border-rgb:\s*var\(--director-border-rgb, 42 42 42\);[\s\S]*?--text-rgb:\s*var\(--director-text-rgb, 255 255 255\);/);
  expect(css).toContain("--accent-rgb: var(--director-accent-rgb, 3 150 255);");
...
it("paints the integrated mount with the active director desk theme", () => {
  const css = readFileSync("src/styles/index.css", "utf8");
...
it("pins the central viewport into a full-bleed director workspace", () => {
  ...
  expect(css).toContain("--left-sidebar-width: 240px;");
  expect(css).toContain("--right-sidebar-width: 380px;");
  expect(css).toContain("--left-sidebar-content-width: 224px;");
  expect(css).toContain("--right-sidebar-content-width: 364px;");
...
it("matches the provided left object panel layout and icon button styling", () => {
  ...
  expect(css).toMatch(/\.object-tree-groups\s*\{[\s\S]*?align-content:\s*start;[\s\S]*?gap:\s*20px;/);
  ...
  expect(css).toMatch(/\.object-row\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\) 28px 28px 28px;[\s\S]*?min-height:\s*45px;[\s\S]*?padding:\s*0 8px;/);
...
it("uses the selected image card capsule style for viewport icon actions", () => {
  ...
  expect(css).toMatch(/\.viewport-toolbar\s*\{[\s\S]*?width:\s*max-content;[\s\S]*?max-width:\s*calc\(100% - 32px\);[\s\S]*?gap:\s*6px;[\s\S]*?padding:\s*6px;/);
  expect(css).toMatch(/\.viewport-toolbar-button\.ui-icon-button\s*\{[\s\S]*?width:\s*40px;[\s\S]*?height:\s*40px;[\s\S]*?border:\s*0;[\s\S]*?padding:\s*0;/);
...
it("renders viewport object labels without an outline stroke", () => {
  ...
  expect(css).toMatch(/\.viewport-object-label-wrapper\s*\{[\s\S]*?zoom:\s*1;/);
  expect(css).toMatch(/@media \(min-width:\s*769px\)\s*\{[\s\S]*?\.viewport-object-label-wrapper\s*\{[\s\S]*?zoom:\s*calc\(1 \/ var\(--app-ui-scale, 1\)\);/);
});

it("lets viewport sensitivity colors inherit the active workbench theme", () => {
  const css = readFileSync("src/styles/index.css", "utf8");
  const start = css.indexOf(".viewport-sensitivity-settings");
  const end = css.indexOf("@media (max-width: 700px)", start);
  const sensitivityStyles = css.slice(start, end);

  expect(sensitivityStyles).toContain("var(--theme-panel-background,");
  expect(sensitivityStyles).toContain("var(--theme-control-background,");
  expect(sensitivityStyles).toContain("var(--theme-accent-icon,");
});

it("maps every workbench theme into the full director desk color system", () => {
  const css = readFileSync("src/styles/index.css", "utf8");
  const workbenchCss = readFileSync("../production-workbench/production-workbench.css", "utf8");
  ...
  for (const token of [
    ...
    expect(css).toContain(`var(--director-${token}-rgb,`);
  }

  for (const themeSelector of [
    ".workbench-body {",
    '.workbench-body[data-workbench-theme="aurora"]',
    '.workbench-body[data-workbench-theme="corona"]',
    '.workbench-body[data-workbench-theme="turquoise"]',
    '.workbench-body[data-workbench-theme="daylight"]',
  ]) {
    const start = workbenchCss.indexOf(themeSelector);
    const end = workbenchCss.indexOf("\n}", start);
    const themeBlock = workbenchCss.slice(start, end);
    expect(themeBlock).toContain("--director-bg-rgb:");
    expect(themeBlock).toContain("--director-panel-rgb:");
    expect(themeBlock).toContain("--director-accent-rgb:");
    expect(themeBlock).toContain("--director-text-rgb:");
  }
});
