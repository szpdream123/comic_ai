...
import { Ni as i, Pi as a, _i as o, t as s } from "./useAppStore-CcUL4Jo0.js";
import { T as c } from "./directorSceneSchema-BcP-NXqL.js";
import { v as l } from "./fileService-zQLozbOU.js";
...
function p({ kind: e, label: t, displayId: n, isBeta: i, nodeId: a, onRename: s }) {
	let c = r(), l = o(e), [p, m] = (0, d.useState)(!1), [h, g] = (0, d.useState)(t), _ = (0, d.useRef)(null);
	(0, d.useEffect)(() => {
...
function h({ nodeId: e, message: t }) {
	let n = s((e) => e.updateNodeDataTransient), [r, i] = (0, d.useState)(null), a = r === t;
	return (0, d.useEffect)(() => {
		let r = setTimeout(() => i(t), m), a = setTimeout(() => {
			let t = s.getState().nodes.find((t) => t.id === e), r = { error: void 0 };
...
function v(e, t, n) {
	let r = s((e) => e.updateNodeData);
	return {
		displayLabel: t.fileName || t.label || n,
		handleRename: (0, d.useCallback)((n) => {
			let i = { label: n };
			t.fileName && (i.fileName = n), r(e, i);
			let a = t.filePath, o = !!(t.imageUrl || t.videoUrl || t.audioUrl);
			if (a && o) {
				let t = s.getState().currentProjectId;
				if (!t) return;
				(async () => {
					let r = await c(a), i = await l(a, n, t);
					if (!i) return;
					let o = s.getState(), u = o.nodes.find((t) => t.id === e)?.data;
					if (!u) return;
...
function V() {
	let { screenToFlowPosition: e } = i(), [t, n] = (0, d.useState)([]), r = (0, d.useRef)(/* @__PURE__ */ new Map()), a = (0, d.useRef)(null), o = (0, d.useRef)(null), c = (0, d.useCallback)(() => {
		n([]);
	}, []), l = (0, d.useCallback)((t, n) => {
		let r = s.getState().nodes, i = new Map(r.map((e) => [e.id, e])), a = null, o = document.querySelector(".react-flow__pane")?.getBoundingClientRect();
		...
		let n = s.getState();
		n.commitToHistory();
		let i = new Set(n.selectedNodeIds), o = i.has(t.id) ? n.nodes.filter((e) => i.has(e.id)) : [t], c = new Set(o.map((e) => e.id)), u = l(t.id, c), d = D(o, u.nodeMap);
		a.current = d ? {
			...u,
			draggedBounds: d,
		...
		let { nodeMap: t, otherXEdges: n, otherXCenters: r, otherYEdges: i, otherYCenters: a } = l(e), s = t.get(e);
		if (!s) {
			o.current = null;
			return;
		}
		...
		let { left: s, top: c, right: l, bottom: u, otherX: d, otherY: f } = a, p = [], m = t, h = r;
		if (i.x !== 0) {
			let e = R([i.x === 1 ? s + t : l - t], d);
			e && (m = i.x === 1 ? e.targetValue - s : l - e.targetValue, p.push({
				kind: "alignment",
				type: "vertical",
...
function G(e) {
	return s((t) => !!e && t.nodes.find((t) => t.id === e)?.draggable === !1);
}
//#endregion
...
function q({ nodeId: e, currentWidth: t, currentHeight: n, minWidth: r = 160, minHeight: i = 120, lockAspectRatio: a = !1, onResizeStart: o, onResizeEnd: c, onResize: l }) {
	...
	return (0, d.useEffect)(() => {
		...
		let t = (t) => {
			...
			let T = C ? s.getState().nodes.find((e) => e.id === C)?.position : void 0;
			m.current = {
				x: t.clientX,
				y: t.clientY,
				w: c,
			...
			let E = !1, D = c, O = l, k = (e) => {
				...
				let { width: k, height: A } = W(t, n, c, l, b, u, d, x, o);
				if (C && w && !f) {
					let e = w.applyResizeSnap(C, k, A, a);
					k = Math.max(u, e.width), A = Math.max(d, e.height);
				}
				k === D && A === O || (E ||= (h?.(), !0), D = k, O = A, S(k, A), C && (r === -1 || i === -1) && T && s.getState().updateNodePositionTransient(C, {
