...
import { Ai as i, F as a, I as o, L as s, R as c, Ui as l, si as u, t as d } from "./useAppStore-BH-MdRLu.js";
import { g as f } from "./directorSceneSchema-D22Qlbpb.js";
import { C as p } from "./fileService-BawXHbsK.js";
import { F as m, Kt as h, L as g, N as _, R as v } from "./useTooltipAutoPlacement-D1FArkVS.js";
import { n as y } from "./rasterImageDimensions-CX1VK2cM.js";
import { a as b, c as x, o as S, s as C, t as w } from "./ResizeHandle-BHdRIIe-.js";
import { a as T, l as E, n as D, o as O, r as k, s as A, t as j } from "./useSourceFileUpload-DYap-2W5.js";
import { t as M } from "./FullscreenOverlay-BTKONk6M.js";
...
function F({ nodeId: e, onUpload: t, onToggleMode: n, previewMode: r, onScreenshot: i, onFullscreen: a }) {
	let o = "ai-panorama", s = A({ nodeType: o }), c = s.registry, l = k({ nodeId: e }), u = d((e) => e.userPresets), f = d((e) => e.addNodeWithEdge), p = (0, N.useCallback)((t) => (n) => {
		n.stopPropagation();
		let r = d.getState().nodes.find((t) => t.id === e);
		if (!r) return;
		let i = r.data?.prompt ?? "", a = d.getState().userPresets, s = g(t, o, i, a);
		if (!s) return;
		let { node: c, edge: l } = m(r, s);
		f(c, l), _(c.id, c.data.prompt, s.postProcess, c.data);
	}, [e, f]), b = {
		upload: (e) => {
			e.stopPropagation(), t?.();
		},
		...
		if (t) return t;
		let n = c.find((t) => t.key === e), i = b[e], a = !n, s = n ? null : v(e, o, u);
		if (!n && !s) return null;
		let d = n ?? {
			key: e,
			label: s.label,
		...
		return e === "toggleMode" ? /* @__PURE__ */ (0, P.jsx)(h, {
...
function W({ id: e, data: t, selected: n }) {
	let r = D(t.status), m = d((e) => e.updateNodeData), h = d((e) => e.updateNodeDataTransient), g = d((e) => e.commitToHistory), _ = d((e) => e.config.theme), v = t.nodeWidth || 280, T = t.nodeHeight || 200, E = (0, N.useRef)(null), O = (0, N.useRef)(null), k = (0, N.useRef)(!1), [A, F] = (0, N.useState)(!1), L = t.previewMode || "image", R = t.panoFullscreen || !1, z = t.imageUrl || t.thumbnailUrl || "", B = (0, N.useCallback)((t, n) => {
		...
		let r = () => {
			let e = s(n, d.getState());
			return e || a(n), e;
		}, i = `全景截图-${Date.now()}`, c = f(i, "png", "panorama-screenshot"), l = await U(t);
		if (!r()) return !1;
		let m = t, h;
		try {
			let e = await p(t, n.projectId, c);
			e && (m = e.assetUrl || t, h = e.filePath);
		} catch {}
		if (!r()) return !1;
		let g = d.getState(), _ = g.nodes.find((t) => t.id === e)?.position ?? {
			x: 0,
			y: 0
		}, y = v, b = l ? Math.round(y / l) : T;
		return g.addNode({
			id: `node-${u()}`,
			type: "ai-image",
			position: {
		...
		}), o(n), g.showToast("截图已创建为图片节点", "success"), !0;
	}, [
		e,
		T,
		v
	]), q = (0, N.useCallback)(async () => {
		let t = d.getState(), n = c(t, e);
		if (!n) {
		...
		try {
			...
			a(n), d.getState().currentProjectId === n.projectId && d.getState().showToast("截图失败", "error");
			return;
		}
		if (!r) {
			let e = s(n, d.getState());
			a(n), e && d.getState().showToast("截图失败", "error");
			return;
		}
		await K(r, n);
	}, [K, e]), ee = (0, N.useCallback)(async ({ dataUrl: t }) => {
		let n = c(d.getState(), e);
		n && await K(t, n);
