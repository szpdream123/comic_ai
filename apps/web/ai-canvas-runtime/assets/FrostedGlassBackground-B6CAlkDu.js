...
var te = class e extends f {
	constructor(e = 1, t = 1, n = 1, r = 2, i = .1) {
		...
		let s = new x(), c = new x(), l = new x(e, t, n).divideScalar(2).subScalar(i), u = this.attributes.position.array, d = this.attributes.normal.array, f = this.attributes.uv.array, p = u.length / 6, m = new x(), h = .5 / a;
...
function M() {
	...
	return (0, C.useEffect)(() => {
		...
		if (!t) return;
		let n = new o();
		n.background = new g(13816785);
		let s = new u(-1, 1, 1, -1, .1, 100);
		s.position.set(0, 0, 14), s.lookAt(0, 0, 0), s.layers.enable(1);
		let l = new v({
			antialias: !0,
			powerPreference: "low-power"
		});
		l.outputColorSpace = d, l.toneMapping = 4, l.toneMappingExposure = .92, l.transmissionResolutionScale = ne, l.setPixelRatio(Math.min(window.devicePixelRatio, j)), l.domElement.setAttribute("aria-hidden", "true"), t.appendChild(l.domElement);
		let f = new b(l), h = new ee(), C = f.fromScene(h, .04).texture;
		n.environment = C, h.dispose(), f.dispose(), n.add(new y(16777215, .012));
		let w = new _(16777215, .035);
		w.position.set(-4, 6, 10), n.add(w);
		let T = new _(16773288, 3.8);
		T.position.set(6, 7, 3), T.layers.set(1), n.add(T);
		let E = new p(16763493, 2819840, .18);
		E.layers.set(1), n.add(E);
		let D = {
			glowCenter: { value: new x() },
			glowRadius: { value: 4 }
		}, M = new r({
			color: 14080213,
			roughness: .64,
			...
			ior: 1.36,
			dispersion: .025,
			attenuationColor: new g(14934748),
		...
		let N = new m({
			...
			metalness: 0,
			envMapIntensity: .02
		}), P = new c(1, 48, 32), F = new i(P, N);
		F.position.z = -2.2, F.layers.set(1), n.add(F);
		let I = null, L = O, R = 0, z = .7, B = R, V = z, H = 0, U = 0, W = !1, G = null, K = () => l.render(n, s), q = (e) => {
			H = 0;
			let t = e - U;
			if (t < A) {
			...
			L = e / t * O, s.left = -L / 2, s.right = L / 2, s.top = O / 2, s.bottom = -12 / 2, s.updateProjectionMatrix();
			let r = L / 2, i = O / 2;
			R = Math.min(r, Math.max(-r, R)), B = Math.min(r, Math.max(-r, B)), z = Math.min(i, Math.max(-6, z)), V = Math.min(i, Math.max(-6, V));
			let o = Math.min(e, t), l = Math.min(5, Math.max(3, o * .003)), u = Math.min(220, Math.max(128, o * .175)), d = Math.max(1, Math.ceil((e + l) / (u + l))), f = Math.max(1, Math.ceil((t + l) / (u + l))), p = O / t, m = u * p, h = l * p;
			G && n.remove(G), I?.dispose();
			let g = m * .72;
			I = new te(m, m, g, 3, 23 / 412.86 * m), M.thickness = g, M.attenuationDistance = m * 3;
			let _ = m + h, v = -((d - 1) * _) / 2, y = (f - 1) * _ / 2;
			G = new S(I, M, d * f);
			let b = new a(), x = 0;
			for (let e = 0; e < f; e += 1) for (let t = 0; t < d; t += 1) b.makeTranslation(v + t * _, y - e * _, 0), G.setMatrixAt(x, b), x += 1;
			G.instanceMatrix.needsUpdate = !0, n.add(G), P.dispose(), P = new c(Math.min(180, Math.max(108, o * .137)) * p, 48, 32), F.geometry = P;
			let C = Math.min(340, Math.max(270, o * .33));
			...
			!e || !n || (l.setPixelRatio(Math.min(window.devicePixelRatio, j)), l.setSize(e, n, !1), re(e, n), K());
